...
- ファイルパス書式（pathFormat）で保存先を柔軟に指定可能（例: `%Y/%m/%d.md`、`%Y-%m-%d.md`など）。
- ObsidianのDaily Notesに直接保存する設定も可能。
- 各メモはUUIDv7で一意に識別され、メモのメタデータに保存される。
- カテゴリごとに独立したファイルまたはディレクトリで管理可能。
...
- `rootDirectory`: memologの保存先ルートディレクトリ（例: `memolog`）。
- `pathFormat`: ファイルパス書式（例: `%Y/%m/%d.md`、`%Y-%m-%d.md`など）。日付フォーマット文字列を使用して保存先を指定。
- `useDirectoryCategory`: ディレクトリでカテゴリを分離して管理するかどうかを切り替えるフラグ。
  - `true`: 各カテゴリに対応するディレクトリを作成し、その中にメモを保存する（例: `memolog/work/2025/11/13.md`）。
...
```markdown
<!-- memo-id: 019abc12-def3-7456-8901-23456789abcd, timestamp: 2025-11-13T10:30:00.000+09:00, category: "work" -->

## 2025-11-13 10:30

メモの内容

<!-- memo-id: 019abc13-def4-7456-8901-23456789abce, timestamp: 2025-11-13T11:00:00.000+09:00, category: "work" -->
...
- 各メモは `<!-- memo-id: ... -->` コメントで始まり、次のコメントまでまたはファイル末尾までが1つのメモ。
- メタデータには以下が含まれる:
  - `memo-id`: UUIDv7形式の一意識別子
  - `timestamp`: ISO 8601形式（タイムゾーンオフセット付き）のタイムスタンプ
  - `category`: メモが属するカテゴリ
  - `parentId`: (オプション) 返信の場合の親メモID
  - `trashedAt`: (オプション) ゴミ箱に移動した日時
  - `permanently-deleted`: (オプション) 完全削除マーカー
- ファイル内のメモは常にタイムスタンプ昇順で管理される。
...
**新機能**: メモとその返信を階層的に表示するスレッド機能を実装。

**主な機能:**

- スレッド表示画面の追加（親メモ + 返信のツリー表示）
- スレッドボタンの追加（返信があるメモに表示）
- スレッド表示でのインライン返信機能
- 2世代以降のスレッドナビゲーション改善（戻るボタンで親スレッドへ）

**UX改善:**

- スレッドボタンをカテゴリ変更ボタンの左に配置
- 返信のカテゴリ変更ボタンを非表示化（返信は親と同カテゴリ）
- 時系列順（古い順）で返信を表示

### 14.2 ルートディレクトリ移行機能の改善

**問題**: ルートディレクトリ変更ボタンが動作しない、設定ファイルが移行時にエラーになる。

**解決策:**

- ObsidianのButtonComponent.onClick()の代わりにネイティブイベントリスナーを使用
- 設定ファイル(memolog-setting.json)を自動上書きしてエラーを回避
- キャンセル/バックアップのみ選択時に入力欄をリセット

### 14.3 設定画面のUI改善

**主な変更:**

- タイトル「memolog 設定」を削除してスッキリした表示に
- 新しい「メモ」タブを追加（基本とカテゴリの間に配置）
- テンプレート設定を「メモ」タブに移動
- 「基本」タブを「ファイルパス」に改名（内容をより明確に）

**タブ構成:**

1. ファイルパス - ルートディレクトリ、pathFormat設定
2. メモ - テンプレート設定
3. カテゴリ - カテゴリ管理
4. ゴミ箱 - ゴミ箱機能設定
5. 高度な機能 - バックアップ、エクスポートなど

### 14.4 ゴミ箱機能の返信投稿対応（基盤実装）

**実装内容:**

- 返信があるメモの削除時に削除マーカー(`permanently-deleted`)を作成
- 削除マーカーの復元拒否処理
- 子孫メモの一括削除/復元機能
