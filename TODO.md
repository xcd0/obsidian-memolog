...
### v0.0.12 UI/UX改善とコード品質向上 ✅ **完了**

**方針**: ユーザー体験の向上とコード品質の大幅改善。
...
- コード品質向上（リファクタリング）
  - [x] memo-manager.ts のリファクタリング
    - [x] memo-search-operations.ts 抽出 (13関数, 65テスト, 100%カバレッジ)
    - [x] ヘルパー関数の活用（updateMemo, updateTodoCompleted）
  - [x] TypeScript/ESLintエラー修正
    - [x] 不要なany型の削除
    - [x] 型安全性の向上

- テスト拡充
  - [x] ゴミ箱機能の統合テスト追加
    - [x] moveToTrash テスト (3件)
    - [x] restoreFromTrash テスト (3件)
  - [x] template-manager.ts の無効テンプレートテスト追加 (2件)
  - [x] 総テスト数: 777件 → 785件 (+8件)
  - [x] 全体カバレッジ: 92.87% → 95.19% (+2.32%)
  - [x] memo-manager.ts カバレッジ: 54.16% → 78.57% (+24.41%)
  - [x] template-manager.ts カバレッジ: 98.18% → 100% ✨

- カバレッジ分析
  - [x] 低カバレッジファイルの調査完了
    - [x] memo-manager.ts (54.16% → 78.57%)
    - [x] vault-handler.ts (90% - 妥当)
    - [x] performance.ts (82.6% - ブラウザAPI依存、妥当)
    - [x] sanitizer.ts (90.32% - 防御的コード、妥当)
    - [x] template-manager.ts (98.18% → 100%)

- ドキュメント整備
  - [x] TODO.md を v0.0.12 に更新
  - [x] README.md を v0.0.12 に更新
  - [x] TESTING_GUIDE.md を更新

---

### v0.0.13 タイムゾーン対応と品質向上 ✅ **完了**

**方針**: タイムゾーン処理の改善とCI環境での品質保証。

- タイムゾーン処理の改善
  - [x] ISO 8601形式(タイムゾーンオフセット付き)でタイムスタンプを保存
    - [x] 形式: `2025-10-31T16:31:43.113+09:00`
    - [x] ユーザーのローカルタイムゾーンで投稿時刻を正確に記録
    - [x] 異なるタイムゾーンのユーザー間でも投稿時刻を正確に共有
    - [x] 夏時間(DST)の切り替えに対応
    - [x] UTC形式との後方互換性を維持

- CI環境互換性の確保
  - [x] ローカル(GMT+9)とCI(GMT+0)の両環境で一貫した動作を実現
  - [x] 環境依存テストの完全修正
    - [x] `setHours()`使用箇所を全てISO 8601形式に置換
    - [x] テンプレートパース処理を正規表現ベースに改善
  - [x] タイムゾーン非依存のテンプレートパース処理実装
    - [x] `createTemplateRegex()`関数を追加 (src/core/memo-helpers.ts)

- コード品質の大幅向上
  - [x] Lintエラー42個を完全解決
    - [x] 型安全性の向上
      - [x] non-null assertion (!) 除去 → 型ガード実装
      - [x] ジェネリック型の明示的指定 (jest.Mock<TReturn, TArgs>)
      - [x] Map型パラメータの明示 (Map<string, string>)
    - [x] コードの不変性保証
      - [x] `let` → `const` 変更
    - [x] 意図の明確化
      - [x] 不要な`async`削除 (4箇所)
    - [x] Jest Mock関連のlint抑制
      - [x] unbound-method警告の適切な抑制 (6箇所)

- テスト品質の向上
  - [x] 全865テスト成功率100%達成
  - [x] Node.js 18.x & 20.x 両環境で成功
  - [x] テストカバレッジ95.19%維持
  - [x] 環境依存性の完全排除
    - [x] timezone-handling.test.ts: 全18テストケース環境非依存化
    - [x] date-range-filter-bug.test.ts: 日付範囲フィルタテスト修正

- ドキュメント整備
  - [x] SPEC.md を v0.0.13 に更新
  - [x] README.md を v0.0.13 に更新
  - [x] TODO.md を v0.0.13 に更新
