...
## v0.0.14 未実装項目（将来のバージョンで対応）

以下の項目は SPEC_v0.0.14.md に記載されていますが、v0.0.14 の必須機能ではありません。
将来のバージョンで実装を検討します。

### スレッド機能の拡張

- [ ] スレッド移動機能（親の変更）
  - UIでドラッグ&ドロップなどで親メモを変更する機能
  - 循環参照チェック機能は実装済み（`wouldIntroduceCycle()`）
  - UI部分のみ未実装
