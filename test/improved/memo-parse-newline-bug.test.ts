import { parseMetadata, parseTextToMemo, memoToText } from "../../src/core/memo-helpers";
...
describe("メモパース - 改行を含むテンプレート(TDD)", () => {
	describe("ステップ1: Red - 再現テストを作成", () => {
		it("改行を含むテンプレートをparseMetadataで正しくパースできる", () => {
			...
			const memoText = `<!-- memo-id: test-id-001, timestamp: 2025-10-31T12:00:00.000Z, category: "work", template: "# %Y-%m-%d %H:%M:%S\\n{{content}}" -->
# 2025-10-31 12:00:00
テストメモ
`;
		...
		it("複数の改行を含むテンプレートもパースできる", () => {
			...
			const memoText = `<!-- memo-id: test-id-003, timestamp: 2025-10-31T12:00:00.000Z, category: "work", template: "# %Y-%m-%d\\n## %H:%M:%S\\n{{content}}" -->
# 2025-10-31
## 12:00:00
複数改行テスト
`;
		...
		it("カンマを含むテンプレートもパースできる", () => {
			...
			const memoText = `<!-- memo-id: test-id-004, timestamp: 2025-10-31T12:00:00.000Z, category: "work", template: "Date: %Y-%m-%d, Time: %H:%M\\n{{content}}" -->
...
End of memo`;
	...
	describe("ステップ3: エッジケースのテスト", () => {
		it("エスケープ文字を含むテンプレートをパースできる", () => {
			...
			const memoText = `<!-- memo-id: test-id-005, timestamp: 2025-10-31T12:00:00.000Z, category: "work", template: "Quote: \\"Hello\\", Tab:\\t, Newline:\\n{{content}}" -->
...
`;
	...
	describe("ステップ4: 実際のユーザーデータでのテスト", () => {
		it("実際のバグケース: template with newline が正しくパースされる", () => {
			...
			const actualBugCase = `<!-- memo-id: 019a2db1-4161-7538-b87e-f0f69c7ee4a4, timestamp: 2025-10-29T01:59:42.689Z, category: "work", template: "# %Y-%m-%d %H:%M:%S\\n{{content}}" -->
# 2025-10-29 10:59:42
qwert
`;
