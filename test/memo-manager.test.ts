...
class MockVaultHandler {
	...
	async initializeTagPair(
		...
		const content =
			`<!-- memolog: {"format": "template", "order": "${order}", "timestamp": "YYYY-MM-DD HH:mm:ss"} -->\n<!-- memolog: start category="${category}" -->\n<!-- memolog: end -->\n`
...
describe("MemoManager", () => {
	...
	beforeEach(() => {
		...
		mockVaultHandler = new MockVaultHandler() // ! VaultHandlerをモックに置き換え。
		;(memoManager as any).vaultHandler = mockVaultHandler // ! statモックをリセット。
