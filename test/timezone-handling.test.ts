...
`;
	...
	describe("後方互換性: UTC形式(Z終端)のタイムスタンプ", () => {
		it("【互換性】UTC形式のタイムスタンプを適切に解析できること", async () => {
			...
			//! 旧形式のメモデータ。
			const utcMemoContent = `<!-- memo-id: test-utc-1, timestamp: 2025-10-28T23:45:37.050Z, category: "work", template: "{{content}}" -->
テストメモ（UTC形式）
`;
		...
		it("【互換性】UTC形式のメモが「今日」フィルターで正しくフィルタリングされること", async () => {
			...
			//! UTC形式のメモデータ。
			const utcMemosContent = `<!-- memo-id: test-utc-yesterday, timestamp: ${yesterday.toISOString()}, category: "work" -->
昨日のメモ（UTC形式）
...
`;
		...
		it("【エッジケース】UTC形式のメモが日付の境界（0:00:00）付近で正しくフィルタリングされること", async () => {
			...
			//! UTC形式のメモデータ。
			const utcMemosContent = `<!-- memo-id: test-utc-yesterday-end, timestamp: ${yesterdayEnd.toISOString()}, category: "work" -->
昨日の最後（UTC形式）
...
`;
		...
		it("【ISO形式】複数タイムゾーンのメモを混在させて「今日」フィルターで正しくフィルタリングされること", async () => {
			...
			const mixedMemoContent = `<!-- memo-id: test-iso-jst-1, timestamp: 2025-10-29T08:00:00.000+09:00, category: "work" -->
JSTメモ(8:00)
...
`;
		...
		it("【ISO形式】複数タイムゾーンのメモを混在させて「一週間」フィルターで正しくフィルタリングされること", async () => {
			...
			const mixedMemoContent = `<!-- memo-id: test-iso-week-8days, timestamp: ${eightDaysAgo.toISOString()}, category: "work" -->
8日前のメモ（UTC形式）
